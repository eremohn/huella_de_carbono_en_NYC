...
bucket = "henry_pf"
yellow_dir = f"datos_ml/MONTH={mes}/data_borough/yellow_trips_day.csv"
s3 = boto3.resource(bucket)
s3_client = boto3.client(bucket)

# Leer archivo csv de taxis amarillos
yellow_obj = s3_client.get_object(Bucket=bucket, Key=yellow_dir)
yellow_string = yellow_obj["Body"].read().decode()
yellow_df_pivot = pd.read_csv(StringIO(yellow_string))
...
for col in ["cant_registros", "sum_distancia", "sum_total", "prom_total"]:
    ...
    yellow_params[col] = yellow_grid.head(1)[["max_depth", "n_estimators", "lags"]].squeeze().to_dict()
...
yellow_predictions = []
for col in yellow_params.keys():
    cols = [x for x in list(yellow_df_pivot.columns) if col in x]
    barrios = [c.split("_")[-1] for c in cols]
    
    yellow_forecaster = ForecasterAutoregMultiSeries(
        regressor=RandomForestRegressor(
            max_depth=yellow_params[col]["max_depth"],
            n_estimators=yellow_params[col]["n_estimators"],
            random_state=42
        ),
        lags=len(yellow_params[col]["lags"]),
        transformer_series=StandardScaler()
    )
    yellow_forecaster.fit(series=yellow_df_pivot[cols])
    yellow_preds = yellow_forecaster.predict(steps=steps)
    
    model_file = f"yellow_taxis_{col}.pkl"
    with open(model_file) as model:
        pickle.dump(yellow_forecaster, model)
    
    boto3.Session().resource('s3').Bucket(bucket).Object(os.path.join(models_folder, model_file)).upload_file(model_file)
    
    # Resumir las predicciones al mes
    if col == "cant_registros":
        series_preds = yellow_preds.sum().round().astype(int)
    elif col == "prom_total":
        series_preds = yellow_preds.mean()
    else:
        series_preds = yellow_preds.sum()
    series_preds.index = barrios
    series_preds.name = col
    yellow_predictions.append(series_preds)
    
# Guardar las predicciones para el mes
preds_df = pd.DataFrame(yellow_predictions).transpose()
